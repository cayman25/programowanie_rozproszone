...
public class PatientController {
    ...
    Mono<ResponseEntity<Patient>> findById(@PathVariable String id){
        return patientService.findById(id).map(ResponseEntity::ok)
                ...
                //.defaultIfEmpty(new ResponseEntity("fff", HttpStatus.NOT_FOUND));
    }
...
//    @PostMapping("patient/add")
//    Mono<ResponseEntity<Patient>> addPatient(@RequestBody Mono<Patient> patient){
//        return patient.map(pat -> patientService.addPatient(pat)).map(body -> ResponseEntity.ok().build());
//    }
